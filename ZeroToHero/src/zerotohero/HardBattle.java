...
public class HardBattle extends JPanel {
    ...
    public HardBattle(Game game) {
        ...
        countDownLabel.setFont(new Font("Skia",Font.PLAIN,36));
        ...
        // Add hero health bar
        heroHealthBar = new JProgressBar(0, BattleScreen.hero.maxHP);
        heroHealthBar.setValue(BattleScreen.hero.maxHP);
        heroHealthBar.setStringPainted(true);
        heroHealthBar.setForeground(new Color(28, 209, 0));
        heroHealthBar.setBackground(Color.black);
        // Add hero mana bar
        heroManaBar = new JProgressBar(0, BattleScreen.hero.maxMana);
        heroManaBar.setValue(BattleScreen.hero.maxMana);
        ...
        defend.addActionListener(e -> {
            buttonDisable(this);
            double temp = BattleScreen.hero.dmgRed;
            BattleScreen.hero.dmgRed += 0.5;
            Timer timer = new Timer(1000, evt -> {
                demonHealthBar.setValue(demon.hp);
                if (demon.hp <= 0) {
                    Timer delay = new Timer(500, fade -> {
        ...
        guardBreak.addActionListener(e -> {
            buttonDisable(this);
            heroImg.moveUpDown();
            demon.hp -= BattleScreen.hero.skill.guardBreak(hero);
            Timer timer = new Timer(1000, evt -> {
                demonHealthBar.setValue(demon.hp);
                if (demon.hp <= 0) {
                    Timer delay = new Timer(500, fade -> {
    ...
    public void heroRegen() {
        BattleScreen.hero.hp+=(int)BattleScreen.hero.hpRegen;
        if (BattleScreen.hero.hp > BattleScreen.hero.maxHP) BattleScreen.hero.hp = BattleScreen.hero.maxHP;
        BattleScreen.hero.mana+=(int)BattleScreen.hero.manaRegen;
        if (BattleScreen.hero.mana > BattleScreen.hero.maxMana) BattleScreen.hero.mana = BattleScreen.hero.maxMana;
        heroHealthBar.setValue(BattleScreen.hero.hp);
        heroManaBar.setValue(BattleScreen.hero.mana);
    }
    ...
    public void heroCrit() {
        ...
        if (num <= BattleScreen.hero.critR) {
            demon.hp -= (int)(BattleScreen.hero.atk*hero.critD);
        }
        else demon.hp -= (int)BattleScreen.hero.atk;
    }
    
    public void buttonDisable(HardBattle screen) {
    ...
    public void buttonEnable(HardBattle screen) {
        screen.attack.setEnabled(true);
        screen.defend.setEnabled(true);
        //str
        if (BattleScreen.hero.str >= 600) {
            screen.guardBreak.setEnabled(true);
            screen.fatalBlow.setEnabled(true);
        }
        else if (BattleScreen.hero.str >= 400) {
            screen.guardBreak.setEnabled(true);
        }
        else if (BattleScreen.hero.str >= 250) {
            screen.guardBreak.setEnabled(true);
        }

        //en
        if (BattleScreen.hero.en >= 600) {
            screen.bloodBath.setEnabled(true);
        }

        //agi
        if (BattleScreen.hero.agi >= 600) {
            screen.sharpEdge.setEnabled(true);
            screen.crossDaggers.setEnabled(true);
            screen.assassinate.setEnabled(true);
        }
        else if (BattleScreen.hero.agi >= 400) {
            screen.sharpEdge.setEnabled(true);
            screen.crossDaggers.setEnabled(true);
        }
        else if (BattleScreen.hero.agi >= 250) {
            screen.sharpEdge.setEnabled(true);
        }

        //magic
        if (BattleScreen.hero.magic >= 600) {
            screen.fireBall.setEnabled(true);
            screen.hex.setEnabled(true);
            screen.manaBurst.setEnabled(true);
        }
        else if (BattleScreen.hero.magic >= 250) {
            screen.fireBall.setEnabled(true);
            screen.hex.setEnabled(true);
        }
        else if (BattleScreen.hero.magic >= 100) {
            screen.fireBall.setEnabled(true);
        }
        
        //mana skills
        if (BattleScreen.hero.mana < (int)(0.1*hero.maxMana)) {
            screen.fireBall.setEnabled(false);
            screen.hex.setEnabled(false);
            screen.manaBurst.setEnabled(false);
        }
        else if (BattleScreen.hero.mana < (int)(0.2*hero.maxMana)) {
            screen.manaBurst.setEnabled(false);
        }
    }
